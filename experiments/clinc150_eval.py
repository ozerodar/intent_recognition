...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-thr", action="store_true", help="Threshold-based evaluation", default=False
    )
    eval_bert, eval_sup, eval_unsup, eval_s2v, use_cuda, eval_thr = vars(
        parser.parse_args()
    ).values()
    ...
    acc_bert_is, acc_bert_oos, acc_us_is, acc_us_oos, acc_s_is, acc_s_oos = (
        -1,
        -1,
        {},
        {},
    ...
    if eval_unsup:
        acc_us_is, _ = evaluate_unsupervised(
            is_x_trn,
            is_y_trn,
            is_x_tst,
            is_y_tst,
        ...
        acc_us_oos, _ = evaluate_unsupervised(
            x_train,
            y_train,
            oos_x_tst,
            oos_y_tst,
    ...
    if eval_sup:
        acc_s_is, _ = evaluate_supervised(
            is_x_trn,
            is_y_trn,
            is_x_tst,
            is_y_tst,
        ...
        acc_s_oos, _ = evaluate_supervised(
            x_train,
            y_train,
            oos_x_tst,
            oos_y_tst,
        ...
        print(
            f"Supervised: In-scope: {' '.join(['Model: {0}. Acc: {1}'.format(k, v) for k, v in acc_s_is.items()])}\n"
            f"Supervised: Out-of-scope: {' '.join(['Model: {0}. Acc: {1}'.format(k, v) for k, v in acc_s_oos.items()])}\n"
        )
    if eval_s2v:
        acc_sent2vec_is = evaluate_sent2vec(
            is_x_trn, is_y_trn, is_x_tst, is_y_tst, is_x_val, is_y_val, dataset="is"
        )
        acc_sent2vec_oos = evaluate_sent2vec(
            x_train, y_train, oos_x_tst, oos_y_tst, oos_x_val, oos_y_val, dataset="oos"
        )
        print(
            f"Sent2vec: In-scope: {acc_sent2vec_is}. Out-of-scope: {acc_sent2vec_oos}"
        )
    ...
    if eval_thr:
        ...
        for thr in THRESHOLDS:
            acc_us_oos_thr = evaluate_unsupervised(
                is_x_trn,
                is_y_trn,
                x_test,
                y_test,
                ...
                emb_models=emb_models,
                thr=thr,
                templates=is_temp_trn,
            )
            acc_s_is_thr = evaluate_supervised(
                is_x_trn,
                is_y_trn,
                x_test,
                y_test,
            ...
            print(
                f"Threshold: {thr}. Unsupervised thr acc: {acc_us_oos_thr}. \nSupervised: {acc_s_is_thr}"
            )
