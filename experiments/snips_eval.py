...
if __name__ == "__main__":
    ...
    if eval_unsup:
        acc_us, _ = evaluate_unsupervised(
            snips_x_trn,
            snips_y_trn,
            snips_x_tst,
            snips_y_tst,
        ...
        print(
            f"Unsupervised: {' '.join(['Model: {0}. Acc: {1}'.format(k, v) for k, v in acc_us.items()])}"
        )
    if eval_sup:
        acc_s, _ = evaluate_supervised(
            snips_x_trn,
            snips_y_trn,
            snips_x_tst,
            snips_y_tst,
        ...
        print(
            f"Supervised: {' '.join(['Model: {0}. Acc: {1}'.format(k, v) for k, v in acc_s.items()])}"
        )
    if eval_s2v:
        acc_s2v = evaluate_sent2vec(
            snips_x_trn,
            snips_y_trn,
            snips_x_tst,
            snips_y_tst,
